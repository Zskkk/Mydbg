...
public class ElfDynamicStructure {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ElfDynamicStructure.class);
    ...
    ElfDynamicStructure(final ElfParser parser, long offset, int size) throws IOException {
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            switch ((int) d_tag) {
                ...
                case DT_INIT_ARRAY:
                    ...
                    debug("initArrayOffset", (int) initArrayOffset);
                    break;
                case DT_INIT_ARRAYSZ:
                    initArraySize = (int) d_val_or_ptr;
                    debug("initArraySize", (int) initArraySize);
                    break;
                case DT_PREINIT_ARRAY:
                    preInitArrayOffset = d_val_or_ptr;
                    debug("preInitArrayOffset", (int) preInitArrayOffset);
                    break;
                case DT_PREINIT_ARRAYSZ:
                    preInitArraySize = (int) d_val_or_ptr;
                    debug("preInitArraySize", (int) preInitArraySize);
                    break;
                case DT_SYMENT:
                    symbolEntrySize = (int) d_val_or_ptr;
                    debug("symbolEntrySize", (int) symbolEntrySize);
                    break;
                case DT_SYMTAB:
                    symbolOffset = d_val_or_ptr;
                    debug("symbolOffset", (int) symbolOffset);
                    break;
                case DT_HASH:
                    hashOffset = d_val_or_ptr;
                    debug("hashOffset", (int) hashOffset);
                    break;
                case DT_GNU_HASH:
                    gnuHashOffset = d_val_or_ptr;
                    debug("gnuHashOffset", (int) gnuHashOffset);
                    break;
                case DT_RELA:
                case DT_REL:
                    relOffset = d_val_or_ptr;
                    debug("relOffset", (int) relOffset);
                    break;
                case DT_RELASZ:
                case DT_RELSZ:
                    relSize = (int) d_val_or_ptr;
                    debug("relSize", (int) relSize);
                    break;
                case DT_RELAENT:
                case DT_RELENT:
                    relEntrySize = (int) d_val_or_ptr;
                    debug("relEntrySize", (int) relEntrySize);
                    break;
                case DT_PLTRELSZ:
                    pltRelSize = (int) d_val_or_ptr;
                    debug("pltRelSize", (int) pltRelSize);
                    break;
                case DT_JMPREL:
                    pltRelOffset = d_val_or_ptr;
                    debug("pltRelOffset", (int) pltRelOffset);
                    break;
                case DT_ANDROID_RELASZ:
                    androidRelASize = (int) d_val_or_ptr;
                    debug("androidRelASize", (int) androidRelASize);
                    break;
        ...
        if (hashOffset > 0) {
            hashTable = new MemoizedObject<HashTable>() {
                @Override
                protected HashTable computeValue() throws ElfException {
        ...
        if (symbolOffset > 0) {
            symbolStructure = new MemoizedObject<ElfSymbolStructure>() {
                @Override
                protected ElfSymbolStructure computeValue() throws ElfException {
...
//
        if (relOffset > 0) {
            ...
            final long fileOffset = relOffset;
            for (int i = 0; i < num_entries; i++) {
                final long relocationOffset = fileOffset + ((long) i * relEntrySize);
                rel[i] = new MemoizedObject<ElfRelocation>() {
                    @Override
                    public ElfRelocation computeValue() {
                        return new ElfRelocation(parser, parser.virtualMemoryAddrToFileOffset(relocationOffset), relEntrySize, symbolStructure.getValue());
                    }
                };
            }
        }
//        for (MemoizedObject<ElfRelocation> rel_item : rel){
//            long off = rel_item.getValue().offset;
//            String symbolN = rel_item.getValue().symbol().getName();
//            int type = rel_item.getValue().type();
//            System.out.println(symbolN+" off: 0x"+Long.toHexString(off)+" type: "+type);
//
//
//        }
...
//
        if (pltRelOffset > 0) {
            ...
            final long fileOffset = pltRelOffset;
            for (int i = 0; i < num_entries; i++) {
                final long relocationOffset = fileOffset + ((long) i * relEntrySize);
            ...
            /*if (logger.isDebugEnabled()){
                for (MemoizedObject<ElfRelocation> r : pltRel){
                    int sym = r.getValue().sym();
                    ElfSymbol symbol = r.getValue().symbol();
...
//	}
    ...
    public MemoizedObject<ElfStringTable> getStringTable(){
        return dtStringTable;
    }

    public MemoizedObject<ElfSymbolStructure> getSymbolTable(){
    ...
    private void debug(String name, int number) {
        logger.debug(String.format("%-20s : 0x%x", "[" + name + "]", number));
    }
}
